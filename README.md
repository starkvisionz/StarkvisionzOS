...
dashboard, settings, the multi-agent loop, nightshift, model replay, and agent
branches are real** — chat streams from the Anthropic Claude API and every turn
is written to an append-only event store, from which the timeline, audit log, and
dashboard projections are rebuilt. The multi-agent loop, nightshift, model replay,
...
prompt through a different model with a real cost diff; and three persona agents
drafting parallel approaches to a decision). The remaining **Lab** views (memory
graph, agent market, negotiation, and the rest) are interactive **simulations** of
...
| **Lab views** | Simulated | Scripted, in-memory interactions — memory graph, market, recovery, counterfactual, truth decay, regret, negotiation, blind spots |
